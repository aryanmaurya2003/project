...
function Sidebar() {
  ...
  return (
    <div className={`h-full ${!isToggle?"p-10":"pt-10"} `}>
      {SideElement.map((items) => (
        ...
        >
          <Link to={items.link} className="ml-3 flex flex-row  ">
            <div
              className={` ${!isToggle?"w-full group-hover:bg-blue-100 mt-2 ":"mt-4 ml-2"}  h-15 flex items-center rounded-sm   ease-in duration-300 ${
                active === items.id ? !isToggle ?"bg-blue-200":"" : ""
              }`}
            >
              <div
                className={`w-11 h-11  flex  justify-center items-center   ${!isToggle?"ml-5 ":"scale-110"}  rounded-md  group-hover:bg-blue-400 group-hover:text-white ease-in duration-200 ${
