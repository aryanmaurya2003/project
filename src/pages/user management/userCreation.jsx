...
function UserCreation() {
  ...
  const validateForm = (data) => {
    ...
    if (!data.email.trim() || !/\S+@\S+\.\S+/.test(data.email)) newErrors.email = true;
    if (!data.password.trim() || data.password.length < 6) newErrors.password = true;
    if (!data.selectRoles) newErrors.selectRoles = true;
          ...
          <div className="flex flex-wrap w-[60%] gap-5">
            {formFields.map((field) => (
              <div key={field.id}>
                ...
                <input
                  ...
                  className={`block ${errors[field.name] ? "border-red-600" : ""} border h-12 indent-2 w-[350px] bg-white placeholder:text-[14px] rounded-md mt-1 focus:outline-none focus:border-2 focus:border-slate-400`}
                />
                {errors[field.name] && (
            ...
            <div className="relative">
              ...
              <select
                ...
                onChange={handleChange}
                className={`text-[14px] ${
                  formData.selectRoles === "" ? "text-slate-400" : "text-black"
